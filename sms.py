...
supported_networks = [234708, 234802, 234808, 234812, 234705,
                      234805, 234811, 234807, 234815, 234703, 234706, 234803,
                      234806, 234810, 234813, 234816, 234809, 234817, 234818]
# **************************************************************************
...
def login(owneremail=owneremail, subacct=subacct, subacctpwd=subacctpwd):
    payload = dict(cmd="login", owneremail=owneremail, subacct=subacct,
                   subacctpwd=subacctpwd)
    global sessionID
    sessionID = requests.post(url, data=payload).text[4:]
    return sessionID

login(owneremail=owneremail, subacct=subacct, subacctpwd=subacctpwd)
...
def sendmessage(message, sendto, sessionid=sessionID, sender=sender):
    ...
    if len(str(sendto)) != 13:
        ...
        payload = dict(sendto=str(sendto), cmd="sendmsg", sessionid=sessionID,
...
if __name__ == "__main__":
    sessionID = login(owneremail=owneremail, subacct=subacct,
                      subacctpwd=subacctpwd)
